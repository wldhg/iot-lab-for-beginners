...
#define SA_ATOMIC_RESTORESTATE                                   \
  uint32_t _sa_saved __attribute__((__cleanup__(SA_iRestore))) = \
      XTOS_DISABLE_LOWPRI_INTERRUPTS

#define ATOMIC() \
  for (SA_ATOMIC_RESTORESTATE, _sa_done = 1; _sa_done; _sa_done = 0)
